...
const DEFAULT_GLOW_COLOR = '208, 174, 99'
...
type ParticleCardProps = PropsWithChildren<{
  className?: string
...
const BentoCardGrid = ({ children, gridRef }: PropsWithChildren<{ gridRef: MutableRefObject<HTMLDivElement | null> }>) => (
  <div
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          ...
          const isRating = card.kind === 'rating'
          const isLocations = card.kind === 'locations'
          const baseClassName = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${isStep ? 'is-step' : ''} ${isRating ? 'is-rating' : ''} ${isLocations ? 'is-locations' : ''}`
          const cardProps = {
            className: baseClassName,
            style: { backgroundColor: card.color, '--glow-color': glowColor } as CSSProperties,
          }

          // Rating card content directly inside the magic-bento card
          if (card.kind === 'rating') {
            return (
              <ParticleCard
                key={index}
                ...
                <div className="magic-bento-card__content locations-card">
                  <div className="locations-info">
                    <div className="office">
                      <div className="office-title">West Palm Beach</div>
                      <div className="office-address">1641 Worthington Rd, Ste 300<br/>West Palm Beach, FL 33409</div>
                    </div>
                    <div className="office">
